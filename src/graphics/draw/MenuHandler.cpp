...
void menuHandler::openChatActionsForNode(uint32_t nodeId)
{
    ...
    o.bannerCallback = [nodeId](int sel) {
        ...
        case kScroll:
            ...
            if (screen)
                screen->showSimpleBanner(g_chatScrollByPress ? "Scroll Btn: ON" : "Scroll Btn: OFF", 1200);
            break;

        case kScrollType:
            g_chatScrollUpDown = !g_chatScrollUpDown;
            if (screen)
                screen->showSimpleBanner(g_chatScrollUpDown ? "Scroll Dir: UP" : "Scroll Dir: DOWN", 1200);
            break;
