...
static void drawFavNodeChatFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (unreadCount > 0) {
        ...
        if (alias)
            std::snprintf(title, sizeof(title), "%s (%s) (%d)", alias, age.c_str(), unreadCount);
        else
            std::snprintf(title, sizeof(title), "%08X (%s) (%d)", (unsigned)nodeId, age.c_str(), unreadCount);
    } else {
        // No unread messages, show normal title
        if (alias)
            std::snprintf(title, sizeof(title), "%s (%s)", alias, age.c_str());
        else
            std::snprintf(title, sizeof(title), "%08X (%s)", (unsigned)nodeId, age.c_str());
    }
    ...
    if (st.scrollIndex > total - 1)
        st.scrollIndex = total - 1;
    if (st.scrollIndex < 0)
        st.scrollIndex = 0;
    // Clamp sel
    if (st.sel < 0)
        st.sel = 0;
    if (st.sel >= visibleRows)
    ...
    for (int row = 0; row < visibleRows; ++row) {
        ...
        if (row == st.sel) {
            ...
            if ((int)base.size() > cap)
                view = base.substr(0, cap - 3) + "...";
            else
                view = base;
        }
        if (needScroll)
...
static void drawChannelChatTabFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (unreadCount > 0) {
        // Show unread count alongside the title
        if (cname)
            std::snprintf(title, sizeof(title), "@%s (%s) (%d)", cname, age.c_str(), unreadCount);
        else
            std::snprintf(title, sizeof(title), "@Channel %u (%s) (%d)", (unsigned)ch, age.c_str(), unreadCount);
    } else {
        // No unread messages, show normal title
        if (cname)
            std::snprintf(title, sizeof(title), "@%s (%s)", cname, age.c_str());
        else
            std::snprintf(title, sizeof(title), "@Channel %u (%s)", (unsigned)ch, age.c_str());
    }
    display->drawString(x, y, title);
    ...
    // Clamp scrollIndex
    if (st.scrollIndex > total - visibleRows)
    ...
    for (int row = 0; row < visibleRows; ++row) {
        ...
        if (e.outgoing)
            ...
            if (sender && sender->has_user && sender->user.long_name[0])
                who = sender->user.long_name;
            else if (e.node) {
                char buf[9];
                std::snprintf(buf, sizeof(buf), "%08X", (unsigned)e.node);
                who = buf;
            } else
                who = "??";
        }
        ...
        for (int r = 0; r < row; ++r) {
            ...
            if (prevIndex >= 0) {
                ...
                if (prevE.outgoing)
                    ...
                    if (prevSender && prevSender->has_user && prevSender->user.long_name[0])
                        prevWho = prevSender->user.long_name;
                    else if (prevE.node) {
                        char buf[9];
                        std::snprintf(buf, sizeof(buf), "%08X", (unsigned)prevE.node);
                        prevWho = buf;
                    } else
                        prevWho = "??";
                }
